...
with st.sidebar:
    ...
    with st.expander("⚖️ 7. 매매 점수 및 필터 (기존 6번)", expanded=False):
        ...
        min_hold_hours = st.slider("최소 보유 시간 (시간)", 0, 24, config.MIN_HOLD_HOURS, 1,
                                   help="매수 후 최소 이 시간 동안은 '분석 기반 매도'를 실행하지 않음 (손절/익절은 실행됨)")


    # --- 설정 저장 버튼 ---
    # [★수정 6★] use_container_width=True -> width='stretch'로 변경
...
with col3:
    ...
    # [★수정 12★] placeholder를 여기서 정의
    placeholder_pnl = st.empty()
    # [★깜빡임 수정★] 봇 실행 시 '0원'으로 초기화되는 코드를 주석 처리합니다.
                    ...
                    if recommend_df is None or recommend_df.empty:
                        ...
                        def style_dataframe(df):
                            ...
                            styled_df = df.style.applymap(style_score, subset=['score']) \
                                            .applymap(style_change, subset=['change_1h', 'change_24h']) \
                                            .format({
                                                'change_1h': '{:.2f}%',
                                                'change_24h': '{:.2f}%',
                                                'rsi': '{:.1f}',
...
if st.session_state.bot_status == "running" and st.session_state.bot:
    ...
    if placeholder_pnl: # placeholder가 정의되었는지 확인
        placeholder_pnl.metric("총 평가손익", "0 원 (0.00%)")